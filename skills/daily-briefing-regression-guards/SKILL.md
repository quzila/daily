...
- `What/Why/So what` は本文の要点を言い換えて説明する。本文冒頭や著者名・公開日・導入段落の貼り付けは禁止。
- 次の形を禁止する:
  - `記事内では「長い本文断片」...`
  - `さらに本文では「長い本文断片」...`
  - `YYYY-MM-DDの収集で ...`
  - `本日（YYYY-MM-DD）の収集では ...`
- 最終レポートは `research/sources/*.json` の `what` / `why` を優先して使い、`selected-with-body.json` の `body_text` から直接説明文を組み立てない。
- `what` / `why` が構造化されていない記事は、公開候補から外すか、source skill を再実行してJSONを補完する。

## Guard 7: 汎用定型文の禁止

- `What/Why/So what` に、他の記事でもそのまま通る汎用文を繰り返さない。
- source skill の出力は `what` / `why` に加えて `so_what` も持たせ、生成器はその構造化メモを優先する。
- `Why` では記事固有の論点を、`So what` では読者の次の判断や検証アクションを記事固有の固有名詞・数値へ接続して書く。
- 3記事以上で同一文が出る場合は、公開前に source skill へ戻して書き直す。
...
6. 擬似要約パターンが0件であることを確認:

```bash
rg -n '記事内では「.{80,}」|さらに本文では「.{80,}」|の収集で|本日（[0-9]{4}-[0-9]{2}-[0-9]{2}）の収集では' \
  reports/daily-news/YYYY-MM-DD/daily-news.md
```

7. 汎用定型文が3記事以上で繰り返されていないことを確認:

```bash
python3 /Users/tatsuru/Desktop/daily/skills/daily-briefing-publish-pages/scripts/publish_daily_briefing.py \
  --date YYYY-MM-DD --timezone Asia/Tokyo --skip-render
```
