...
2. `$daily-news-md-to-html` で同日の `index.html` を生成する。
   - 生成は当日 `research/selected-with-body.json` と `research/sources/official-*.json` を統合した内容を前提にする（前日Markdown流用禁止）。
3. 公開前にフォーマット品質を確認する（`### 記事1` のような連番見出しが残っていないか、`What/Why/So what` が全記事にあり、各ブロックが十分な文量か）。
   - 併せて `## 今日のハイライト（3選）` が存在し、前日と同一文面の流用になっていないことを確認する。
...
- publish時に品質ゲートを実行する。次を満たさない場合は公開を停止する:
  - `## 今日のハイライト（3選）` が存在し、3項目あること
  - ハイライト3項目が前日と完全一致しないこと（当日内容へ更新）
  - AI記事 8件以上 / 半導体記事 5件以上
  - 各記事に `What/Why/So what` が存在し、各項目が最低文量を満たす
  - `What/Why/So what` に本文断片の長い引用や `記事内では「...」` 形式の擬似要約がないこと
  - `What/Why/So what` に3記事以上で繰り返す汎用定型文がないこと
  - `## ソース一覧` セクションにメタ情報とURLが存在する
  - Zenn/note記事URLは正規形式かつ到達可能（404不可）
- DNS制限環境で `socket.gaierror` が発生する場合、Zenn/noteはURL形式チェックのみで継続し、到達確認失敗だけを理由に停止しない。
