...
各ソースSkillの出力は次のJSON構造に揃える。`what` / `why` / `so_what` のいずれかが欠ける記事は統合せず、source skill を再実行して補完する。
...
- `daily-news.md` の本文生成は `research/sources/*.json` の構造化 `what` / `why` を優先し、`body_text` の長文断片から説明文を組み立てない。
- 最終 `daily-news.md` は各記事に `What/Why/So what` を含める。
- `What/Why/So what` は `research/sources/*.json` の構造化フィールドをそのまま核にし、全記事に共通の汎用説明文を足してごまかさない。
- `## 今日のハイライト（3選）` を必須にし、3項目すべて当日の選定記事に基づいて書く（前日と同一文面を禁止）。
- `What/Why/So what` は短文禁止。各項目は最低60文字以上で、結論だけでなく根拠（事実・背景・影響）を明示する。
- `What/Why/So what` に本文冒頭の長い引用や `記事内では「...」` 形式の擬似要約を残さない。
- `What/Why/So what` に3記事以上で同一の定型文を繰り返さない。記事固有の固有名詞・数値・判断点を必ず入れる。
